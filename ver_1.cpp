...
//Constantes para un proyectil en la tierra	
const double g = 9.81; // Gravedad en la Tierra en m/s^2
...
double Misil[100] {
    //Aquí definimos las variables necesarias para hacer las operaciones.
  double m,x,y,z,vx,vy,vz,fx,fy,fz,fco,L,t,wx,wy,wz; //Definición de parámetros
  
   start(double L0,double m0,double t0,double x0,double y0,double z0,double vx0,double vy0,double vz0); // Definición de las condicio
   force(void);//Function that calculates the forces over the bullet
   force2(void);//Function that calculates the forces over the bullet without considering the Coriolis and centrifugal forces
   midpoint(double dt);//Nunerical method
...
//Entrada de variables m=masa de proyectil, x=, y=, z=, vx=, vy=, vz=, fx=, fy=, fz=, fco=, L=, t=, wx=, wy=, wz=
...
//Centrifugal force -> F=m*w x (w x r), r -> circumference radius described by the missile due to Earth´s rotation
 ...
 Missile::force(void) {//It calculates the total force over the missile considering Coriolis and centrifugal forces.
    double rx,ry,rz;
...
int main () {
  ...
  // ofstream f("missile1.txt"),h("missile2.txt"),c("command");   ///--------------------- COSAS PARA GNUPLOT VER DESPUÉS <-------------------------------
  
  Missile Gustav1;
  Missile Gustav2;
  double a,b,vel,ori,vx,vy,vz,delta,reach,t0,x0,y0,z0,vx0,vy0,vz0, Dtt, f0;
  Printf("Introduzca las siguientes condiciones iniciales (use un unico numero):");
  Printf("\n");
  Printf("Elevación angular para el lanzamiento en grados (min=0, max=90)= ");
  Scanf("%ld", &b);
  b=b*M_PI/180;
  Printf("\n");
  Printf("Angulo Azimuthal en grados (N=90, E=0, S=-90, W=180) = ");
  Scanf("%ld", &ori);
  ori=ori*M_PI/180;
  Printf("\n");
  ...
  if(a>0)printf("The launch took place in the northern hemisphere.");
  if(a<0)printf("The launch took place in the southern hemisphere.");
  if(a==0)printf("The launch took place in the Ecuador line");
  printf(" ");
  printf("The horizontal range was "<<reach<<" meters");
  printf(" ");
  printf("The horizontal deviation was "<<delta<<" meters");
  printf(" "<<endl;
  printf("The flight time was: "<<Gustav1.Gett()<<" seconds");
  printf(" "<<endl;
  printf("WARNING: -x axis -> West, +x axis -> East");
  printf("             -y axis-> South, +y axis -> North");
  printf(" ");
